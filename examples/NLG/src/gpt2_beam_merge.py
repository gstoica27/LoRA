...
if __name__ == '__main__':
    ...
    base_checkpoints = [
        "/srv/flash1/pramesh39/LoRA/examples/NLG/trained_models/GPT2_M/e2e/gpt2_md_lora_e2e.pt",
        "/srv/flash1/pramesh39/LoRA/examples/NLG/trained_models/GPT2_M/webnlg/gpt2_md_lora_webnlg.pt"
    ]
    
    lm_net = GPT2LMModel(config)
    print('loading model pretrained weight.')
    lora_state_dict = torch.load(base_checkpoints[0], map_location=torch.device('cpu')) 
    for ckpt in base_checkpoints[1:]:
        cp = torch.load(ckpt, map_location=torch.device('cpu'))
        for key in cp:
            lora_state_dict[key] += cp[key]
    
    lora_state_dict = {key:lora_state_dict[key]/len(base_checkpoints) for key in lora_state_dict.keys()}
    lm_net.load_weight(lora_state_dict)
    lm_net = lm_net.cuda()
