...
class DataTrainingArguments:
    ...
    task_names: List[str] = field(
        ...
        metadata={"help": "The names of the tasks to train on: " + ", ".join(task_to_keys.keys())},
    )
    ...
    def __post_init__(self):
        if self.task_names is not None:
            for idx, task_name in enumerate(self.task_names):
                ...
                self.task_names[idx] = task_name.lower()
...
class ModelArguments:
    ...
    lora_paths: List[str] = field(
        # default=[],
        default_factory=list,
        metadata={"help": "The file paths of LoRA parameters."},
    )
...
def main():
    ...
    if data_args.task_names is not None:
        ...
        datasets = [load_dataset("glue", task_name) for task_name in data_args.task_names]
    else:
    ...
    #         num_labels = len(label_list)
    num_labels = 1000 # TODO: REMOVE THIS AND UNCOMMENT FROM # Labels
    is_regression = "stsb" in data_args.task_names
    ...
    config = AutoConfig.from_pretrained(
        ...
        finetuning_task=data_args.task_names[0],
    ...
    trainable_params = []
    classifiers = [deepcopy(model.classifier) for _ in model_args.lora_paths]
    model.classifier = torch.nn.Identity() # Overwrite classifier to remove it (We will use our own)
    
    if model_args.apply_lora:
        if model_args.lora_paths is not None:
            # Load all state dicts
            lora_state_dicts = []
            for lora_path in model_args.lora_paths:
                state_dict = torch.load(lora_path)
                logger.info(f"Apply LoRA state dict from {lora_path}.")
                logger.info(state_dict.keys())
                lora_state_dicts += [state_dict]
            
            # Merge parameters
            lora_state_dict = {}
            for key in lora_state_dicts[0].keys():
                if 'classifier' not in key:
                    merged_param = torch.stack([sd[key] for sd in lora_state_dicts], dim=0).sum(0)
                    lora_state_dict[key] = merged_param
            
            # lora_state_dict = torch.load(model_args.lora_path)
            model.load_state_dict(lora_state_dict, strict=False)
        trainable_params.append('lora')
        # Add classifier components
        for classifier, lora_sd in zip(classifiers, lora_state_dicts):
            classifier.weight.data = lora_sd['classifier.weight']
            classifier.bias.data = lora_sd['classifier.bias']
    ...
    if data_args.task_names is not None:
        task2data_info = defaultdict(lambda: dict())
        for task_name in data_args.task_names:
            task2data_info[task_name] = list(task_to_keys[task_name])
    ...
    # TODO: Uncomment and Fix for Non-GLUE
    # label_to_id = None
    # if (
    ...
    def preprocess_function(examples, sentence1_key, sentence2_key):
        # Tokenize the texts
        args = (
        ...
        # Map labels to IDs (not necessary for GLUE tasks) TODO: Uncomment and Fix for Non-GLUE
        # if label_to_id is not None and "label" in examples:
        #     result["label"] = [(label_to_id[l] if l != -1 else -1) for l in examples["label"]]
        return result
    
    preprocessed_datasets = []
    for dataset, info in zip(datasets, task2data_info.values()):
        preprocessor = lambda ds: preprocess_function(ds, info[0], info[1])
        dataset = dataset.map(preprocessor, batched=True, load_from_cache_file=not data_args.overwrite_cache)
        preprocessed_datasets += [dataset]
    ...
    if training_args.do_eval:
        eval_datasets = []
        for dataset, task_name in zip(datasets, data_args.task_names):
    ...
    if training_args.do_predict or data_args.task_names is not None or data_args.test_file is not None:
        test_datasets = []
        for dataset, task_name in zip(datasets, data_args.task_names):
            if "test" not in dataset and "test_matched" not in dataset:
                raise ValueError("--do_predict requires a test dataset")
            test_dataset = dataset["test_matched" if task_name == "mnli" else "test"]
    ...
    if data_args.task_names is not None:
        metrics = [load_metric("glue", task_name) for task_name in data_args.task_names]
    # TODO: When datasets metrics include regular accuracy, make an else here and remove special branch from
    # compute_metrics

    # You can define your custom compute_metrics function. It takes an `EvalPrediction` object (a namedtuple with a
    # predictions and label_ids field) and has to return a dictionary string to float.
    def compute_metrics(p: EvalPrediction, metric):
    ...
    if training_args.do_eval:
        ...
        tasks = data_args.task_names
        # eval_datasets = [eval_dataset]
        if "mnli" in data_args.task_names:
            tasks.append("mnli-mm")
            mnli_index = tasks.index('mnli')
            eval_datasets.append(datasets[mnli_index]["validation_mismatched"])

        for eval_dataset, task, classifier, metric in zip(eval_datasets, tasks, classifiers, metrics):
            trainer.model.classifier = classifier # Overwrite identity with the relevant classifier
            trainer.model.num_labels = classifier.bias.shape[0]
            trainer.compute_metrics = lambda p: compute_metrics(p, metric) # Overwrite compute_metrics function
